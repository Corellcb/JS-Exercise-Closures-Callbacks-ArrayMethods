...
function firstNamesAllCaps(runners) {
  const nameDrop = function (element) {
    return element.first_name.toUpperCase();
  }
  
  return runners.map(nameDrop);
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let sizeCheck = function(element) {
    return element.shirt_size === tShirtSize;
  }
  
  return runners.filter(sizeCheck);
}
...
function tallyUpDonations(runners) {
  let donationSum = function(total, element) {
    return total + element.donation;
  }

  // let sum = runners.reduce((a, b) => a.donations + b.donations, 0);
  
  return runners.reduce(donationSum, 0);
}
